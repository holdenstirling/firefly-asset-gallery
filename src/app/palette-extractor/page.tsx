...
export default function PaletteExtractorPage() {
  ...
  const extractPalette = useCallback(
    async (nextUrl: string, nextSwatchCount: SwatchCount) => {
      ...
      await new Promise<void>((resolve) => {
        ...
        image.onload = () => {
          try {
            ...
            if (!canvas) {
              setStatus("error");
              setErrorMessage(
                "Canvas is unavailable. Reload the page and try again."
              );
              resolve();
              return;
            }
        ...
        image.onerror = () => {
          setStatus("error");
          setErrorMessage(
            "Could not load this image URL. Check the link and try again."
          );
          resolve();
        };
      ...
      <header className="flex shrink-0 items-center justify-between border-b border-border px-6 py-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Pull dominant colors from any image URL and save them as a Style
            preset.
          </p>
        </div>
      </header>
      ...
      <div className="grid flex-1 grid-cols-1 gap-6 overflow-y-auto p-6 lg:grid-cols-2">
        <section className="rounded-xl border border-border bg-card p-5 shadow-sm">
          ...
          {isLoading ? (
            ...
            <div className="mt-4 flex aspect-[4/3] items-center justify-center rounded-xl border border-dashed border-border bg-background/40 px-6 text-center text-sm text-muted-foreground">
              {status === "error"
                ? errorMessage
                : "Extract a palette to preview the source image."}
            </div>
          )}
        </section>
        ...
        <section className="rounded-xl border border-border bg-card p-5 shadow-sm">
          ...
          <div className="mt-5 flex flex-col gap-4">
            ...
            <Button
              variant="gradient"
              onClick={() => void extractPalette(url, swatchCount)}
            >
              Extract palette
            </Button>
            ...
            {savedStyleId && (
              <p className="rounded-lg border border-border bg-background/50 px-3 py-2 text-sm text-muted-foreground">
                ...
                to review the preset. Use Apply there to push it to the gallery.
              </p>
            )}
          </div>
        </section>
